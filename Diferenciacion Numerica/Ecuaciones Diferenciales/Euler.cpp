...
// ============================================================================
// EULER METHOD - Método de Euler para ecuaciones diferenciales ordinarias
...
void euler_metodo(double a, double b, int n, double y0) {
    ...
    // Método de Euler: y[i+1] = y[i] + h*f(x[i], y[i])
    for (int i = 0; i < n; i++) {
        double k = f(x[i], y[i]);
        x[i+1] = x[i] + h;
        y[i+1] = y[i] + h * k;
    ...
    cout << "\n" << string(100, '=') << endl;
    cout << "MÉTODO DE EULER - Ecuación diferencial: dy/dx = -2xy" << endl;
    ...
    // Lambda para decidir qué filas mostrar: primeras 3, últimas 2, cada 10%, y puntos cercanos a valores de interés
...
int main() {
    ...
    cout << "\n" << string(100, '=') << endl;
    cout << "MÉTODO DE EULER PARA ECUACIONES DIFERENCIALES ORDINARIAS" << endl;
    cout << string(100, '=') << endl;
