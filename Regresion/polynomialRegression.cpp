...
int main(void) {
	...
	do {
		...
		if (rows < degree + 1) {
			cout << "La regresi�n no es posible para el polinomio deseado" << endl;
			flag = 1;
		} else {
			flag = 0;
		}
...
void assembleMatrix(double nodes[MAXROWS][MAXCOLUMNS], double matrix[MAXROWS][MAXCOLUMNS2], double b[MAXROWS], int rows,
	...
	void details(double nodes[MAXROWS][MAXCOLUMNS], double x[MAXROWS], int rows, int degree) {
		...
		cout << "DETALLES" << endl;
		cout << "--------" << endl;
		double syx;    // Desviaci�n est�ndar-(error est�ndar estimado)
		double sr;    // Error, residuo - (suma de los cuadrados de los residuos)
		double st;    // Error medio- (suma total de los cuadrados)
		double r2;    // Coeficiente de determinaci�n
		double r;    // Coeficiente de correlaci�n
		double ymedia;    // Media de y
		double sum;
		double ecm;
		
		// C�lculo de media de y
		ymedia = 0;
		for (int index = 0; index < rows; index++){
			...
			cout << "Error cuadr�tico medio: " << ecm << endl;
			cout << "Desviaci�n est�ndar (Error est�ndar estimado): " << syx << endl;
			cout << "Error medio (suma total de cuadrados): " << st << endl;
			cout << "Coeficiente de determinaci�n: " << r2 << endl;
			cout << "Coeficiente de correlaci�n: " << r << endl;
	}
