...
void mostrarTabla(double a[FILAS][2], int filas, int columnas){
	cout << "Matriz Aumentada:" << endl;
	for (int i = 0; i < filas; ++i) {
		for (int j = 0; j < columnas - 1; ++j) {
			cout << a[i][j] << "\t";
		}
		cout << "| " << a[i][columnas-1] << endl;
	}
	cout << endl;
}

int main(int argc, char *argv[]) {
	double m[FILAS][2]={{0, 0}, {1, 2.1}, {2, 3.8}, {3, 5.1}, {4, 5.9}, {5, 6.3}};
...
void lagrange(double m[FILAS][2], int fila) {
    double x=2.5; // Valor a interpolar
